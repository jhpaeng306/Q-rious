...
def preCostFunction(weight,bias,dataSets):
    loss=0
    for i in range(dataSets.shape[0]):
        makeResult = preCircuitPair(weight, bias, dataSets[i, 0:4], dataSets[i, 4:8])[0]
        loss += makeResult
...
def preTrain(cost,weight,bias,pairs,minBatchSize,datan,steps):
    for i in range(steps):
        batch_size=minBatchSize*((i//10)+1)
        batchLoop=1
        for j in range(batchLoop):
            if (batch_size*(j+1)>=datan):
                batchX = pairs[batch_size*j:datan]
            else:
                batchX = pairs[batch_size*j:batch_size*(j+1)]
            weight, bias, _ = optimizer.step(cost, weight, bias, batchX)
            batchX = pairs[np.random.randint(0, datan, (batch_size,))]
            print(i,j)
            ...
            for i1 in range(bias.shape[0]):
                ...
                if (i1<bias.shape[0]-1):
                    ...
                    file.write(']')
            file.close()
            print(cost(weight, bias, batchX))

def postTrain(cost,weight,bias,postParam,pairs,index,minBatchSize,datan,steps):
    ...
    for i in range(steps):
        ...
        for j in range(batchLoop):
            if (batch_size*(j+1)>=datan):
                batchX = pairs[batch_size*j:datan]
                batchY = index[batch_size*j:datan]
            else:
                batchX = pairs[batch_size*j:batch_size*(j+1)]
                batchY = index[batch_size*j:datan]
            _, _, postParam, _, _ = optimizer.step(cost, weight, bias, postParam, batchX, batchY)
            batchX = pairs[np.random.randint(0, datan, (batch_size,))]
            print(i,j)
...
postPairs = np.array(postData[:, 0:8],requires_grad=False)
postIndex = np.array(postData[:,8],requires_grad=False)
...
bias = np.random.randn(postLayerN,qubitn*2)
postParam = np.random.randn(qubitn*2*3)
...
preTrain(preCostFunction,weight,bias,prePairs,minBatchSize,preDatan,steps)
postTrain(postCostFunction,weight,bias,postParam,postPairs,postIndex,minBatchSize,postDatan,steps)

preBatchX = prePairs[np.random.randint(0, preDatan, (testBatchSize,))]
getSample=np.random.randint(0, postDatan, (testBatchSize,))
postBatchX = postPairs[getSample]
postBatchY = postIndex[getSample]
print(preCostFunction(weight, bias, preBatchX))
